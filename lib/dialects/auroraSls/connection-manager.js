    ...
    const connection = {
      ...
      execute: (sql, parameters, handler) => {
        ...
        parameters.forEach((parameter, i) => {
          const escapedParameter = SqlString.escape(parameter, null, 'mysql');
          setStatements.push(`SET @pc${i} = ${escapedParameter};`);
          usingParams.push(`@pc${i}`);
        });
        const preparedStatement = `
        PREPARE stmt1 FROM '${sql}';
        ${setStatements.join('\n')}
        EXECUTE stmt1 USING ${usingParams.join(',')};
        DEALLOCATE PREPARE stmt1;`;
        console.log(preparedStatement);
        query({ sql: preparedStatement }, handler);
      }
    };
