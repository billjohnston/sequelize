...
class Query extends AbstractQuery {
  ...
  zipAwsResults ({ sqlStatementResults = [] }) {
    return sqlStatementResults.map(({ resultFrame }) => {
      const records = _.get(resultFrame, ['records'], [])
      const columnMetadata = _.get(
        resultFrame, ['resultSetMetadata', 'columnMetadata'], []
      )
      return records.map(({ values }) => values.reduce(
        (acc, valueMap, i) => {
          const { label, name, typeName } = columnMetadata[i]
          return {
            ...acc,
            [label]: valueMap[this.typeNameValueMap[typeName]]
          }
        },
        {}
      ))
  ...
  formatResults(data) {
    ...
    if (this.isShowIndexesQuery()) {
      const zippedData = this.zipAwsResults(data)[0]
      console.log(zippedData)
      return this.handleShowIndexesQuery(zippedData);
    }
    if (this.isCallQuery()) {
      return data[0];
    }
    ...
    if (this.isVersionQuery()) {
      return data.sqlStatementResults[0].resultFrame.stringValue;
    }
